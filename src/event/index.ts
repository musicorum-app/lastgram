...
class EventEngine {
    ...
    queueEvent(id: string, userID: string, listener: EventListenerIdentifier) {
        debug('event.queueEvent', `queueing new event under id ${id}`)
    ...
    async dispatchEvent(id: string, ctx: MinimalContext) {
        ...
        debug('event.dispatchEvent', `id ${id} path: ${keyValue}`)
        ...
        const commandEngine = await import('../commands/index.js').then(m => m.commandRunner)
