...
import { getArtistScrobble, upsertArtistScrobbles } from './artist-scrobbles.js'
...
export const checkIfUserHasCrown = async (groupId: string, fmUsername: string, artistMbid: string) => {
  const query = `
      SELECT *
      FROM crowns
      WHERE groupId = ?
  ...
  `
  const r = await client.execute(query, [groupId, fmUsername, artistMbid], { prepare: true })
  return !!r.rows[0]
...
export const createCrown = async (groupId: string, artistMbid: string, fmUsername: string, playCount: number) => {
  const query = `
  ...
  const r = await client.execute(query, [groupId, artistMbid, fmUsername, playCount, n, n], { prepare: true })
  return r?.first()
}

export const updateCrownPlays = async (groupId: string, artistMbid: string, fmUsername: string, playCount: number) => {
...
export const transferCrownOwnership = async (crown: any, fmUsername: string, playCount: number) => {
  await client.execute(`
          UPDATE crowns
          SET fmUsername    = ?,
              playCount     = ?,
          ...
          WHERE groupId = ?
            AND artistMbid = ?;
      `, [fmUsername, playCount, now(), (crown.switchedtimes || 0) + 1, crown.groupid, crown.artistmbid],
    { prepare: true }
  ).catch((e) => {
...
export const appendToPastCrownHolders = async (groupId: string, artistMbid: string, fmUsername: string, playCount: number) => {
  return await client.execute(`
      INSERT INTO crown_holders (groupId, artistMbid, fmUsername, playCount, createdAt)
      VALUES (?, ?, ?, ?, ?);
  `, [groupId, artistMbid, fmUsername, playCount, now()], { prepare: true })
}

export const tryToStealCrown = async (groupId: string, artistMbid: string, fmUsername: string, artistName: string, playCount: number): Promise<CrownResult> => {
  // first, we must get the fmUser's artist scrobble
  let artistScrobble = await getArtistScrobble(fmUsername, artistMbid).catch((e) => {
  ...
  if (crown && crown.fmusername.toLowerCase() === fmUsername.toLowerCase()) {
    if (!crown.playcount || playCount > crown.playcount) await updateCrownPlays(groupId, artistMbid, fmUsername, artistScrobble.playcount)
    return { success: false, reason: 'alreadyHas', crown }
  }

  if (!crown) {
    // if it doesn't exist, the user can get the crown
    await createCrown(groupId, artistMbid, fmUsername, artistScrobble.playcount)
    return { success: true }
  }
  ...
  if (artistScrobble.playcount > crown.playcount) {
    ...
    const artistScrobbleFromCrown = await getArtistScrobble(crown.fmusername, artistMbid).catch((e) => {
    ...
    if (artistScrobble.playcount > artistScrobbleFromCrown.playcount) {
      ...
      await appendToPastCrownHolders(groupId, artistMbid, crown.fmusername, crown.playcount).catch((e) => {
      ...
      await updateCrownPlays(groupId, artistMbid, fmUsername, artistScrobble.playcount)
...
export const getCountPastCrownHolders = async (groupId: string, artistMbid: string) => {
  const query = `
